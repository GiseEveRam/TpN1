...
app.put("/peliculas/:id", (req,res) =>{
    ...
    writeData(data);
    res.json({ message: "Peliculas updated sucessfully"});
});

app.delete("peliculas/:id", (req, res) => {
    const data = readData();
    const id = parseInt(req.params.id);
    data.peliculaIndex = data.peliculas.findIndex((pelicula) => pelicula.id === id);
    data.peliculas.splice(peliculaIndex, 1);
    writeData(data);
    res.json({ message : "peliculas deleted sucessfully"});
});
